...
public class SpawnBuildings : EditorWindow {
	...
	{ 
		...
		for (int i = 0; i < length; i++) 
			{ 
				...
				go = (GameObject)Instantiate(insGo, parent.transform.position + new Vector3( parent.transform.position.x + Random.Range(0,positionJitter), parent.transform.position.y,parent.transform.position.z*2 + i * width ),parent.transform.rotation,parent.transform);
	...
	void OnGUI() {
		...
		EditorGUILayout.Space ();
		house0 = ((GameObject)EditorGUI.ObjectField (new Rect (3, 150, position.width - 12, 20), "Find GameObject", house0, typeof(GameObject)));
		...
		EditorGUILayout.Space ();
		length = EditorGUILayout.IntSlider("Length",length,0, 200);
		EditorGUILayout.Space ();
